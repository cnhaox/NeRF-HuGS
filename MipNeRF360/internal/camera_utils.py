...
def cast_ray_batch(
    ...
    heights: _Array,
    widths: _Array,
    distortion_params: Optional[Mapping[str, float]],
    camtype: ProjectionType = ProjectionType.PERSPECTIVE,
  ...
  """
  pixtocams, camtoworlds, pixtocam_ndc = cameras
  ...
  origins, directions, viewdirs, radii = pixels_to_rays(
      pixels.pix_x_int,
      pixels.pix_y_int,
  ...
  h, w = heights[cam_idx], widths[cam_idx]
  pix_coords = xnp.stack([
    (pixels.pix_x_int.astype(xnp.float32) + 0.5) / w, 
    (pixels.pix_y_int.astype(xnp.float32) + 0.5) / h
  ], axis=-1)
  ...
  return utils.Rays(
      ...
      static_mask=pixels.static_mask,
      near=pixels.near,
      far=pixels.far,
      embed_idx=pixels.embed_idx,
...
def cast_spherical_rays(camtoworld: _Array,
  ...
  ray_args = (origins, directions, viewdirs, radii)
