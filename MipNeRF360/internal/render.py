...
def compute_dual_alpha_weights(density1, density2, tdist, dirs, opaque_background=False):
  """Helper function for computing alpha compositing weights."""
  t_delta = tdist[..., 1:] - tdist[..., :-1]
  delta = t_delta * jnp.linalg.norm(dirs[..., None, :], axis=-1)
  density1_delta = density1 * delta
  density2_delta = density2 * delta
  density_delta = (density1 + density2) * delta
  if opaque_background:
    # Equivalent to making the final t-interval infinitely wide.
    density1_delta = jnp.concatenate([
      density1_delta[..., :-1], jnp.full_like(density1_delta[..., -1:], jnp.inf)
    ], axis=-1)
    density2_delta = jnp.concatenate([
      density2_delta[..., :-1], jnp.full_like(density2_delta[..., -1:], jnp.inf)
    ], axis=-1)
    density_delta = jnp.concatenate([
      density_delta[..., :-1], jnp.full_like(density_delta[..., -1:], jnp.inf)
    ], axis=-1)
  
  alpha1 = 1 - jnp.exp(-density1_delta)
  alpha2 = 1 - jnp.exp(-density2_delta)
  alpha = 1 - jnp.exp(-density_delta)
  trans = jnp.exp(-jnp.concatenate([
    jnp.zeros_like(density_delta[..., :1]),jnp.cumsum(density_delta[..., :-1], axis=-1)
  ], axis=-1))
  weights1 = alpha1 * trans
  weights2 = alpha2 * trans
  weights = alpha * trans
  return weights1, weights2, weights
...
def volumetric_rendering_combined_color(rgbs_static,
                                        rgbs_transient,
                                        bg_rgbs,
                                        weights_static,
                                        weights_transient,
  ...
  Returns:
    rendering: a dict containing an rgb image of size [batch_size, 3], and other
      visualizations if compute_extras=True.
  """
  acc = weights_combined.sum(axis=-1)
  bg_w = jnp.maximum(0, 1 - acc[..., None])  # The weight of the background.
  rgb_static = (weights_static[..., None] * rgbs_static).sum(axis=-2)
  rgb_transient = (weights_transient[..., None] * rgbs_transient).sum(axis=-2)
  rgb = rgb_static + rgb_transient + bg_w * bg_rgbs

  return rgb, rgb_static, rgb_transient