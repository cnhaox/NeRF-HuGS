...
def main(unused_argv):
  ...
  model, state, render_eval_pfn, _, _ = train_utils.setup_model(config, key)
  if config.finetune_enable:
    state, _, _ = train_utils.setup_finetune_model(config, model, state)
    checkpoint_dir = path.join(config.checkpoint_dir, "finetune")
  else:
  ...
  while True:
    ...
    step = int(state.step)
    if config.finetune_enable: step += int(config.max_steps)
    if step <= last_step:
    ...
    for idx in range(dataset.size):
      ...
      rays = batch.rays
      train_frac = np.clip(step / config.max_steps, 0, 1)
      ...
      if config.eval_save_output and (config.eval_render_interval > 0):
        if (idx % config.eval_render_interval) == 0:
          utils.save_img_u8(rendering['rgb'],
                            path_fn(f'{idx:04d}_color.png'))
          utils.save_img_u8(gt_rgb,
                            path_fn(f'{idx:04d}_gt.png'))
          utils.save_img_u8(rendering['rgb_cc'],
                            path_fn(f'{idx:04d}_colorcc.png'))

          for key in ['distance_mean', 'distance_median']:
            if key in rendering:
              vis_depth = vis.visualize_cmap(rendering[key], rendering['acc'], cm.get_cmap('turbo'), curve_fn=depth_curve_fn)
              utils.save_img_u8(vis_depth,
                                path_fn(f'{idx:04d}_{key}.png'))
              utils.save_img_f32(rendering[key],
                                 path_fn(f'{idx:04d}_{key}.tiff'))
          ...
          utils.save_img_f32(rendering['acc'], path_fn(f'{idx:04d}_acc.tiff'))
    ...
    if (config.eval_save_output and (not config.render_path) and
      ...
      with utils.open_file(path_fn(f'metric_mean_{step}.txt'), 'w') as f:
        f.write(f'render time: {np.mean(render_times)}\n')
        for name in metrics[0]:
          f.write(f'{name}: {np.mean([m[name] for m in metrics])}\n')
        for name in metrics_cc[0]:
          f.write(f'cc_{name}: {np.mean([m[name] for m in metrics_cc])}\n')
